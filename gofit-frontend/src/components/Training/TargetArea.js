...
const TargetArea = () => {
  const params = useParams(); //target area
  const navigate = useNavigate();
  
  const [trainings, setTrainings] = useState(null);
  const [query, setQuery] = useState('');
  const [searchParam] = useState(["duration", "name", "type"]);

  useEffect(() => {
    const fetchTrainingByTargetArea = async () => {
      const response = await CustomAxios.get('http://localhost:8088/training/targetArea/' + params.targetArea);
      setTrainings(response?.data);
      console.log(response?.data);
    } 

    fetchTrainingByTargetArea().catch(error => {
      console.log(error);
    });
  }, []);
  ...
  return (
    <Container className="mt-5 pt-4 pb-5">
      <Row className="mb-4">
        <h1>Choose your {params.targetArea} training</h1>
      </Row>
      <Row className="mb-3">
    ...
    {trainings != null && search(trainings).map((training, index) => {
      return <div className="card mb-3" key={`${training.id}__${index}`} onClick={() => startTrainingHandler(training.id)}>
        <div className="row g-0">
          ...
          <div className="col-md-8">
              <div className="card-body">
                ...
                <p className="card-text">Type: {training.type}</p>
